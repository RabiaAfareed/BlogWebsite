...
const blogPosts = [
  {
    id: 1,
    title: "Introduction React Components",
    image: "/react-components.jpeg",
    content: `React components are the building blocks of any React application. They allow you to divide the UI into independent, reusable pieces and think about each piece in isolation.
    
    Each component is a JavaScript function or class that returns a piece of JSX code. Components can be simple or complex, depending on the requirements. They can also accept input data (known as props) and manage their own state, making them dynamic and interactive.
    
    There are two main types of components: functional components and class components. Functional components are simple JavaScript functions that return JSX. With the introduction of hooks, functional components can also manage state and side effects, making them more powerful. Class components, on the other hand, extend from the React.Component class and can manage state and lifecycle methods.
    
    Reacts component-based architecture promotes code reusability and maintainability. By breaking the UI into smaller components, developers can easily update or debug specific parts of the application without affecting the rest.`,
    author: "Jane Doe",
    comments: [
      {
        id: 1,
        content: "This explanation is really clear and helpful!",
        author: "John Smith",
        replies: [
          { id: 1, content: "Absolutely! React components make everything modular.", author: "Alice Johnson" },
          { id: 2, content: "I love how React handles UI updates efficiently.", author: "Bob Brown" }
        ]
      }
    ]
  },
  ...
  {
    id: 3,
    title: "JavaScript EcmasScript (ES6) Features",
    image: "/jimg.jpeg",
  ...
  },
  {
    ...
    image: "/graphimg.png",
    content: `sanity is a query language for your API, and a server-side runtime for executing queries by using a type system you define for your data. It provides a more flexible and efficient alternative to traditional REST APIs, allowing clients to request exactly the data they need.
    
    One of the main advantages of sanity is its ability to reduce over-fetching and under-fetching of data. With REST APIs, clients often receive more data than they need or have to make multiple requests to get the required data. sanity solves this problem by allowing clients to specify the structure of the response, ensuring that they receive only the necessary data in a single request.
    
    sanity schemas define the types and relationships between data in the API. A schema consists of type definitions that describe the shape of the data and the operations that can be performed on it. This allows clients to understand the available data and query it in a consistent manner.
    
    Queries in sanity are written using a simple and intuitive syntax. Clients can specify the fields they need and nest related fields to retrieve complex data structures in a single query. This eliminates the need for multiple endpoints and reduces the complexity of the client-side code.
    
    Mutations in sanity are used to modify data. They allow clients to perform create, update, and delete operations while specifying the structure of the response. This ensures that clients receive immediate feedback about the changes made to the data.
    
    sanity also supports real-time updates through subscriptions. Subscriptions allow clients to receive updates whenever the data changes, enabling real-time applications and improving the user experience.
    
    In addition to its flexibility and efficiency, sanity provides strong typing and introspection capabilities. This allows developers to validate queries at compile time and provides powerful tools for exploring and understanding the API.
    
    Many popular frameworks and libraries, such as Apollo and Relay, provide tools and integrations for building sanity APIs and clients. These tools simplify the development process and help developers implement best practices.
    
    In summary, sanity is a powerful and flexible query language for APIs that provides a more efficient and consistent way to request and manipulate data. Its ability to reduce over-fetching and under-fetching, along with its strong typing and real-time capabilities, make it an excellent choice for modern web development.`,
    author: "Pam Beesly",
    comments: [
      {
        id: 9,
  ...
  {
    id: 9,
    title: "Web perfomance optimization",
    image: "/reduximg.jpeg",
